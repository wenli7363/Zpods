...
RemoteFileDialog::RemoteFileDialog(QWidget *parent, const QList<QPair<QString,QString>>& fileList) :
    QDialog(parent),
    ui(new Ui::RemoteFileDialog)
{
    ...
    ui->remoteSrcTable->horizontalHeader()->setSectionResizeMode(QHeaderView::Stretch);
    ...
    {
        QString fileName = fileList.at(row).first + "/" + fileList.at(row).second;
