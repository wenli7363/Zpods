...
void MainWindow::enableFileFilter()
{
    ...
    connect(ui->filterChkBox, &QCheckBox::stateChanged, this, [&]() {
        ...
        if (filterChecked)
        {
            ...
            connect(filterDialog, &FileFilterDialog::sentFilterConfig, this,
                    [this](std::shared_ptr<FilterConfig> filterconfig) {
...
void MainWindow::enableRemote()
{
    ...
    connect(ui->remoteChkBox, &QCheckBox::stateChanged, this, [&]() {
        ...
        else
        {
            ...
            loginDialog->daemonThread->stopThread();
        }
    });
}
...
void MainWindow::enableSrcBtn()
{
    connect(ui->srcBtn, &QPushButton::clicked, this, [this]() {
        ...
        disconnect(button, SIGNAL(accepted()), dialog,
                   SLOT(accept())); //使链接失效
        connect(button, SIGNAL(accepted()), dialog,
                SLOT(slot_myAccetp())); //改成自己的槽
...
void MainWindow::enableDeleteBtn()
{
    connect(ui->deleteBtn, &QPushButton::clicked, this, [&]() {
        QList<QListWidgetItem*> selectedItems =
            ui->srcListWidget->selectedItems();
...
void MainWindow::enableTargetBtn()
{
    connect(ui->targetBtn, &QPushButton::clicked, this, [&]() {
        QString targetPath = QFileDialog::getExistingDirectory(
            this, "选择目的路径", QDir::homePath());
...
void MainWindow::enableEncryptChkBox()
{
    connect(ui->encryptChkBox, &QCheckBox::stateChanged, this,
            [this](int state) {
                ...
                {
                    // 复选框勾选上时，弹出密码输入对话框
                    QString psw = QInputDialog::getText(
                        this, "输入密码", "请输入密码:", QLineEdit::Password);
                    if (!psw.isEmpty())
                    {
...
void MainWindow::handleBackup(BackupOptions backupOptions)
{
    ...
    {
        backupOptions.config.crypto_config =
            zpods::CryptoConfig(backupOptions.password);
    }
        ...
        do
        {
            ...
            for (const auto& item : backupOptions.src_path_list)
            {
                ...
                let backup_file_path =
                    backupOptions.config.current_pod_path.parent_path();

                if (backupOptions.remoteChk)
                {
                    let status =
                        loginDialog->user.upload_pods(backup_file_path.c_str());
    ...
    connect(backupThread, &QThread::finished, backupThread,
            &QObject::deleteLater);

    // monitor thread
    connect(backupThread, &BackupThread::startedSignal, this,
            [this, backupThread](ThreadInfo info) {
                ...
                QStringList record;
                record << QString::number(info.taskID) << info.filename
                       << info.cmps << info.encrypt << info.syn << info.remote
                       << info.period;
    ...
    connect(backupThread, &BackupThread::finishedSignal, this,
            [this, backupThread](ThreadInfo info) {
                ...
                {
                    if (ui->ingTable->item(row, 0)->text() ==
...
void MainWindow::enableRSrcBtn()
{
    connect(ui->rSrcBtn, &QPushButton::clicked, this, [&]() {
        QString fileName = QFileDialog::getExistingDirectory(this, "Open File",
                                                             QDir::homePath());
...
void MainWindow::enableThreadStopBtn()
{
    connect(ui->threadStopBtn, &QPushButton::clicked, this, [this]() {
        ...
        {
            uint taskId =
                ui->ingTable->item(selectedItem->row(), 0)->text().toUInt();
...
void MainWindow::enableLogoutBtn()
{
    connect(ui->logoutBtn, &QPushButton::clicked, this,
            [this]() { ui->remoteChkBox->setCheckState(Qt::Unchecked); });
}
...
void MainWindow::enabledeleteBtnDL()
{
    connect(ui->deleteBtnDL, &QPushButton::clicked, this, [this]() {
        QList<QListWidgetItem*> selectedItems =
            ui->srcListWidgetDL->selectedItems();
        for (QListWidgetItem* item : selectedItems)
        {
    ...
    connect(ui->downloadBtn, &QPushButton::clicked, this, [this]() {
        ...
        for (const QString& item : this->DLset)
        {
            ...
            if (parts.size() == 2)
            {
                ...
                loginDialog->user.download_pod(pods, pod,
                                               this->dowloadTargetPath);
            }
            else
            {
